...
class OtherFilesPage extends StatefulWidget {
  ...
  @override
  State<OtherFilesPage> createState() => _OtherFilesPageState();
}
...
class _OtherFilesPageState extends State<OtherFilesPage> {
  ...
  void showEditOtherFileDialog(
    GetOtherFilesResponseData assessment,
  ) {
    showDialog(
      context: context,
      builder: (context) {
  ...
  Future<void> deleteOtherFile(int id) async {
    try {
      ...
      if (mounted) {
        const snackBar = SnackBar(content: Text('File Deleted Successfully'));
        ScaffoldMessenger.of(context).showSnackBar(snackBar);
      }
    } catch (e) {
      if (mounted) {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: const Center(child: Text("Search Other Files")),
          shape: const RoundedRectangleBorder(
            borderRadius: BorderRadius.all(Radius.circular(20.0)),
          ),
          content: StatefulBuilder(
                ...
                (item) {
                  ...
                  return DataRow(
                    cells: <DataCell>[
                      ...
                      DataCell(
                        IconButton(
                          onPressed: () => showEditOtherFileDialog(
                            item,
                          ),
