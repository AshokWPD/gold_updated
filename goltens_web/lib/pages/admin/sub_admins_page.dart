  ...
  void showEditSubAdminDialog(GetUsersResponseData user) {
    showDialog(
      ...
      builder: (context) {
        ...
        return AlertDialog(
          contentPadding: const EdgeInsets.only(left: 25, right: 25),
          title: const Center(child: Text("Edit SubAdmin")),
          shape: const RoundedRectangleBorder(
            borderRadius: BorderRadius.all(Radius.circular(20.0)),
          ),
          content: StatefulBuilder(
  ...
  void showSearchDialog() {
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          contentPadding: const EdgeInsets.only(left: 25, right: 25),
          title: const Center(child: Text("Search SubAdmins")),
          shape: const RoundedRectangleBorder(
            borderRadius: BorderRadius.all(Radius.circular(20.0)),
          ),
          content: StatefulBuilder(
  ...
  void showCreateSubAdminDialog() {
    showDialog(
      ...
      builder: (context) {
        ...
        return AlertDialog(
          contentPadding: const EdgeInsets.only(left: 25, right: 25),
          title: const Center(child: Text("Create SubAdmin")),
          shape: const RoundedRectangleBorder(
            borderRadius: BorderRadius.all(Radius.circular(20.0)),
          ),
          content: StatefulBuilder(
              ...
              rows: subAdmins
                  .map(
                    (user) => DataRow(
                      ...
                      selected: selectedSubAdmins.contains(user.id),
                      onSelectChanged: (isItemSelected) {
                        setState(() {
                          if (isItemSelected == true) {
                            selectedSubAdmins.add(user.id);
                          } else {
                            selectedSubAdmins.remove(user.id);
                          }
                        });
                      },
                    ),
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      ElevatedButton.icon(
                                        ...
                                        label: const Text('Create SubAdmin'),
                                      ),
                                    ],
                                  ),
                                ],
