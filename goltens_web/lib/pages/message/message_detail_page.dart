...
class UserMessageDetailPage extends StatefulWidget {
  const UserMessageDetailPage({super.key});

  @override
  State<UserMessageDetailPage> createState() => _UserMessageDetailPageState();
}

class _UserMessageDetailPageState extends State<UserMessageDetailPage> {
  @override
...
class _MessageDetailAdminPageState extends State<MessageDetailAdminPage>
  ...
  Future<void> exportCsvFile() async {
    ...
    html.AnchorElement(href: url)
      ..setAttribute("download", 'read-status-${message.id}.csv')
      ..click();

    html.Url.revokeObjectUrl(url);
  }

  Future<void> showReadInfoDialog() async {
    final settings = ModalRoute.of(context)!.settings;
    final args = settings.arguments as MessageDetailPageArgs;
    final message = args.message;
